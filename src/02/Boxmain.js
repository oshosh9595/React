    ...
    for(let mv of mvlist) {
        ...
        divTags.push(
            <div className="rowDiv" key={mv.movieCd} onClick={() => handleDivClick(mv)}>
                ...
                <span className="col" id="colmoviNm">{mv.movieNm}</span>
                <span className="col" id="colsalesAmt">{parseInt(mv.salesAmt).toLocaleString('ko-KR')}</span>
                <span className="col" id="colrankIntenrankInten">{
                    inten
                }</span>
            </div>
        );
    }
    // handleDivClick 값이 전달될때는 콜백함수 써야한다.
    return (
        <div className="content">
            <div className="rowDiv0">
                <span className="col" id="colrank">순위</span>
                <span className="col" id="colmoviNm">영화명</span>
                <span className="col" id="colsalesAmt">매출액</span>
                <span className="col" id="colrankIntenrankInten">증감률</span>
            </div>
            {divTags}
            <div className="rowDay">
                <strong>[{dspmv.movieNm}] 개봉일 : {dspmv.openDt};</strong>
            </div>
        </div>
    );
