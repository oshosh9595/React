...
const Frcst = () => {
    ...
    frcstFourDt : 넷째날예보일시
    */
    const [info, setInfo] = useState(); //setInfo리엑트가관리하는 훅 함수 가져오는거임 만드는거 아니다. [변수명, 변수를바꾸는함수(고정돤함수)]
    const items = [
        {
  ...
  // object 는 key 값과등 여러가지가져가고 배열은 정해진데이터만 
  // 배열은 [] 오브젝트는 {}
    ...
    const showInfo = (seldt) => {
        ...
        //<li key={`${v}-${seldt}`} style={{color:'blue'}}>{v}</li>  
        console.log(infoArry);

        setInfo(infoArry);

    } //map 사용법 변수.map(() => ) 괄혼안에 리턴이가능한 콜백함수
